package com.awrtechnologies.carbudgetsales.hlistview.widget;
...
import com.awrtechnologies.carbudgetsales.hlistview.util.ViewHelperFactory;
import com.awrtechnologies.carbudgetsales.hlistview.util.ViewHelperFactory.ViewHelper;
import com.awrtechnologies.carbudgetsales.hlistview.util.v11.MultiChoiceModeListener;
import com.awrtechnologies.carbudgetsales.hlistview.util.v11.MultiChoiceModeWrapper;
...
public abstract class AbsHListView extends AdapterView<ListAdapter> implements ViewTreeObserver.OnGlobalLayoutListener,
	...
	/**
	 * The list will automatically scroll to the bottom when a com.awrtechnologies.carbudgetsales.data set change notification is received and only if the last item is
	...
	/**
	 * Layout to sync as a result of a com.awrtechnologies.carbudgetsales.data change. Restore mSyncPosition to have its top at mSpecificLeft
	 */
	 ...
	 * and the value holds the last known position in the com.awrtechnologies.carbudgetsales.adapter for that id.
	 */
	...
	/**
	 * The com.awrtechnologies.carbudgetsales.adapter containing the com.awrtechnologies.carbudgetsales.data to be displayed by this view
	 */
	protected ListAdapter mAdapter;

	/**
	 * If mAdapter != null, whenever this is true the com.awrtechnologies.carbudgetsales.adapter has stable IDs.
	 */
	...
	/**
	 * The com.awrtechnologies.carbudgetsales.data set used to store unused views that should be reused during the next layout to avoid creating new ones
	 */
	 ...
	 * across a com.awrtechnologies.carbudgetsales.data set change. If the item isn't found it will be unselected.
	 */
	...
	/**
	 * This view is in transcript mode -- it shows the bottom of the list when the com.awrtechnologies.carbudgetsales.data changes
	 */
	...
	/**
	 * The select child's view (from the com.awrtechnologies.carbudgetsales.adapter's getView) is enabled.
	 */
	...
	/**
	 * An estimate of how many pixels are between the top of the list and the top of the first position in the com.awrtechnologies.carbudgetsales.adapter, based on the
	 * last time we saw it. Used to hint where to draw edge glows.
	 */
	private int mFirstPositionDistanceGuess;

	/**
	 * An estimate of how many pixels are between the bottom of the list and the bottom of the last position in the com.awrtechnologies.carbudgetsales.adapter, based on
	...
	/**
	 * Track the item count from the last time we handled a com.awrtechnologies.carbudgetsales.data change.
	 */
	 ...
	 * before the sync happens, we can return this existing com.awrtechnologies.carbudgetsales.data rather than losing it.
	 */
	...
	public AbsHListView( Context context, AttributeSet attrs, int defStyle ) {
		...
		if( null != array ) {
			...
			choiceMode = array.getInt( R.styleable.AbsHListView_android_choiceMode, AbsListView.CHOICE_MODE_NONE );
			array.recycle();
	...
	public void setAdapter( ListAdapter adapter ) {
		if ( adapter != null ) {
			...
			if ( mChoiceMode != AbsListView.CHOICE_MODE_NONE && mAdapterHasStableIds &&
	 ...
	 * {@link #CHOICE_MODE_NONE} and the com.awrtechnologies.carbudgetsales.adapter has stable IDs. ({@link ListAdapter#hasStableIds()} == {@code true})
	 * 
	 * @return A new array which contains the id of each checked item in the list.
	 */
	public long[] getCheckedItemIds() {
		if ( mChoiceMode == AbsListView.CHOICE_MODE_NONE || mCheckedIdStates == null || mAdapter == null ) {
			return new long[0];
		}
	...
	public void setItemChecked( int position, boolean value ) {
		...
		if ( android.os.Build.VERSION.SDK_INT >= 11 ) {
			if ( value && mChoiceMode == AbsListView.CHOICE_MODE_MULTIPLE_MODAL && mChoiceActionMode == null ) {
		...
		// Do not generate a com.awrtechnologies.carbudgetsales.data change while we are in the layout phase
	...
	public boolean performItemClick( View view, int position, long id ) {
		...
		if ( mChoiceMode != AbsListView.CHOICE_MODE_NONE ) {
			...
			if ( mChoiceMode == AbsListView.CHOICE_MODE_MULTIPLE
					|| ( android.os.Build.VERSION.SDK_INT >= 11 && mChoiceMode == AbsListView.CHOICE_MODE_MULTIPLE_MODAL && mChoiceActionMode != null ) ) {
	 ...
	 * com.awrtechnologies.carbudgetsales.adapter and the position of the visible items inside the com.awrtechnologies.carbudgetsales.adapter. This provides a stable scrollbar as the user navigates
	...
	public Parcelable onSaveInstanceState() {
		...
		ss.filter = null;
		ss.inActionMode = android.os.Build.VERSION.SDK_INT >= 11 && mChoiceMode == AbsListView.CHOICE_MODE_MULTIPLE_MODAL
				&& mChoiceActionMode != null;
	...
	public void onRestoreInstanceState( Parcelable state ) {
		...
		if ( android.os.Build.VERSION.SDK_INT >= 11 ) {
			if ( ss.inActionMode && mChoiceMode == AbsListView.CHOICE_MODE_MULTIPLE_MODAL && mMultiChoiceModeCallback != null ) {
				mChoiceActionMode = startActionMode( (MultiChoiceModeWrapper)mMultiChoiceModeCallback );
			}
		}
	...
	protected float getLeftFadingEdgeStrength() {
		...
		if ( count == 0 ) {
			...
			return left < getPaddingLeft() ? -( left - getPaddingLeft() ) / fadeLength : fadeEdge;
		}
	}
	...
	protected float getRightFadingEdgeStrength() {
		...
		if ( count == 0 ) {
			...
			return right > width - getPaddingRight() ?
					( right - width + getPaddingRight() ) / fadeLength : fadeEdge;
		}
	}
	...
	/**
	 * Get a view and have it show the com.awrtechnologies.carbudgetsales.data associated with the specified position. This is called when we have already discovered
	 ...
	 *           false if otherwise.
	 * 
	 * @return A view displaying the com.awrtechnologies.carbudgetsales.data associated with the specified position
	 */
	@SuppressLint ( "NewApi" )
	...
	private class PerformClick extends WindowRunnnable implements Runnable {
		...
		@Override
		public void run() {
			// The com.awrtechnologies.carbudgetsales.data has changed since we posted this action in the event queue,
	...
	public boolean onTouchEvent( MotionEvent ev ) {
		...
		if ( !mIsAttached ) {
			...
			// Since we rely on being attached to get com.awrtechnologies.carbudgetsales.data set change notifications,
		...
		switch ( action & MotionEvent.ACTION_MASK ) {
			...
			case MotionEvent.ACTION_MOVE: {
				...
				if ( mDataChanged ) {
					// Re-sync everything if com.awrtechnologies.carbudgetsales.data has been changed
					// since the scroll operation can query the com.awrtechnologies.carbudgetsales.adapter.
					layoutChildren();
				}
	...
	/**
	 * Fires an "on scroll state changed" event to the registered {@link com.awrtechnologies.hlistview.widget.AbsHListView.OnScrollListener}, if any.
	...
	private class FlingRunnable implements Runnable {
		 ...
		 */
		private final com.awrtechnologies.carbudgetsales.hlistview.widget.OverScroller mScroller;
	...
	/**
	 * Smoothly scroll to the specified com.awrtechnologies.carbudgetsales.adapter position. The view will scroll such that the indicated position is displayed.
	 * 
	 * @param position
	 *           Scroll to this com.awrtechnologies.carbudgetsales.adapter position.
	 */
	...
	/**
	 * Smoothly scroll to the specified com.awrtechnologies.carbudgetsales.adapter position. The view will scroll such that the indicated position is displayed, but it
	 * will stop early if scrolling further would scroll boundPosition out of view.
	 * 
	 * @param position
	 *           Scroll to this com.awrtechnologies.carbudgetsales.adapter position.
	 * @param boundPosition
	 *           Do not scroll if it would move this com.awrtechnologies.carbudgetsales.adapter position out of view.
	 */
	 ...
	 *         mResurrectToPosition. Values are pinned to the range of items available in the com.awrtechnologies.carbudgetsales.adapter
	 */
	...
	protected void handleDataChanged() {
		...
		if ( mChoiceMode != AbsListView.CHOICE_MODE_NONE && mAdapter != null && mAdapter.hasStableIds() ) {
		...
		if ( count > 0 ) {
				...
				switch ( mSyncMode ) {
					case SYNC_SELECTED_POSITION:
						if ( isInTouchMode() ) {
							...
							// See if we can find a position in the new com.awrtechnologies.carbudgetsales.data with the same
			...
			if ( !isInTouchMode() ) {
				// We couldn't find matching com.awrtechnologies.carbudgetsales.data -- try to use the same position
	 ...
	 * 
	 * @see com.awrtechnologies.hlistview.widget.AbsHListView.RecycleBin
	 * @see com.awrtechnologies.hlistview.widget.AbsHListView.RecyclerListener
	 */
	 ...
	 * 
	 * @see com.awrtechnologies.hlistview.widget.AbsHListView.RecycleBin
	 * @see com.awrtechnologies.hlistview.widget.AbsHListView#setRecyclerListener(com.awrtechnologies.hlistview.widget.AbsHListView.RecyclerListener)
	 */
	 ...
	 * could potentially be used by the com.awrtechnologies.carbudgetsales.adapter to avoid allocating views unnecessarily.
	 * 
	 * @see com.awrtechnologies.hlistview.widget.AbsHListView#setRecyclerListener(com.awrtechnologies.hlistview.widget.AbsHListView.RecyclerListener)
	 * @see com.awrtechnologies.hlistview.widget.AbsHListView.RecyclerListener
	 */
	public class RecycleBin {
		...
		/**
		 * Unsorted views that can be used by the com.awrtechnologies.carbudgetsales.adapter as a convert view.
		 */
		...
		/**
		 * Makes sure that the size of mScrapViews does not exceed the size of mActiveViews. (This can happen if an com.awrtechnologies.carbudgetsales.adapter does not
		 * recycle its views).
		 */
		@SuppressLint ( "NewApi" )
