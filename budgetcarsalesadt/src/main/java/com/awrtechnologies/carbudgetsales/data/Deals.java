...
@Table(name = "Deals")
public class Deals extends Model {
	...
	public static List<Deals> getAllById(int catid) {
		return new Select().from(Deals.class).where("categoryId=?", catid)
				.execute();
	}

	public static List<Deals> getAllByIdString() {
		return new Select().from(Deals.class)
				.execute();
	}
	public static Deals getAllByDealId(String dealId) {
		return new Select().from(Deals.class).where("dealId=?", dealId)
				.executeSingle();
	}
